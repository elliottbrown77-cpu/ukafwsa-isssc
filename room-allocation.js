  ...
  async function load({sync=false}={}){
    ...
    try{
      ...
      const [requests,rooms,allocations,locations,attendees]=await Promise.all([
        ...
        client.from('attendees').select('id,title_rank,first_name,surname,email,mobile,category,display_company').eq('event_id',eventId).order('surname').order('first_name')
      ]);
