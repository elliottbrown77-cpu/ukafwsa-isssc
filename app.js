...
async function loadInvoices(){
 ...
 const results=await Promise.all([
  ...
  supabase.from('invoices').select('id,event_id,attendee_id,billing_account_organisation_id,invoice_type,invoice_reference,status,issue_date,due_date,payment_link,purchase_order_reference,payment_terms_days,notes,net_total,vat_total,gross_total,sent_at,paid_at,approved_by,approved_at,issued_by,issued_at,paid_by,payment_reference,created_at,updated_at').eq('event_id',EVENT_ID).order('created_at',{ascending:false}).limit(100),
 ...
 [state.financeReadiness,state.financeSummaries,state.financeAttendees,state.financeOrganisations,state.financeSponsors,state.financeInvoices,state.financeRates]=results.slice(0,7).map(result=>result.data||[]);
 state.financeBillingSettings=results[7].data||null;
...
function renderFinanceInvoiceDetail(){
 ...
 const workflowAction=invoice.status==='ready_for_review'?`<button class="btn btn-primary" type="button" data-invoice-action="approved" ${lines.length?'':'disabled'}>Confirm invoice</button>`:invoice.status==='approved'?`<button class="btn btn-primary" type="button" data-invoice-action="issued" ${state.financeBillingSettings?.billing_configuration_confirmed?'':'disabled'}>Mark as issued</button>`:'';
 const workflowMessage=invoice.status==='awaiting_billing_update'?'Rebuild this invoice after resolving the billing update.':invoice.status==='ready_for_review'?'Check every charge line before confirming. Confirmation permanently locks the financial snapshot.':invoice.status==='approved'?(state.financeBillingSettings?.billing_configuration_confirmed?'The financial snapshot is locked and the invoice is ready to issue.':'Confirm the event billing settings before issuing this invoice.'):invoice.status==='issued'?'The invoice has been issued. Record payment only after the funds have been received.':invoice.status==='paid'?'Payment has been recorded and this invoice is complete.':'Rebuild the draft when the attendee is ready.';
